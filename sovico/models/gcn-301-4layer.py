...
class GCN():
    ...
    def __init__(self, input_dim, adj_matrix, gcn_hiddens):
        ...
        # third layer
        self.W3 = tf.Variable(init([gcn_hiddens[1], gcn_hiddens[2]]))
        self.L3 = tf.matmul(self.normalized_matrix, self.L2) # shape: (N, gcn_hiddens[1])
        self.L3 = tf.matmul(self.L3, self.W3) # shape: (N, gcn_hiddens[2])
        self.L3 = tf.nn.relu(self.L3)
        
        # fourth layer
        self.W4 = tf.Variable(init([gcn_hiddens[2], gcn_hiddens[3]]))
        self.L4 = tf.matmul(self.normalized_matrix, self.L3) # shape: (N, gcn_hiddens[2])
        self.L4 = tf.matmul(self.L4, self.W4) # shape: (N, gcn_hiddens[3])
        self.L4 = tf.nn.relu(self.L4)
        
        # last layer (FC layer)
        self.W5 = tf.Variable(init([gcn_hiddens[3], num_classes]))
        self.L5 = tf.matmul(self.L4, self.W5) # shape: (N, num_classes)
        
        # loss
        self.loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.L5, labels=self.y)
        self.loss = self.loss * self.mask # apply mask
        self.loss = tf.reduce_mean(self.loss)
        
        # prediction
        self.pred = tf.argmax(self.L5, axis=1)
    ...
    def fit(self, data, save_dir, learning_rate=0.001, epochs=500, patience=10):
        ...
        y_pred = tf.argmax(self.L5, axis=1)
...
def get_modelname(window_size, after, gcn_hiddens):
    ...
    return 'gcn301_4layer_window{}_after{}_dims{}'.format(window_size, after, gcn_hiddens)
