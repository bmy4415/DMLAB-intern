...
class GCN():
    ...
    def __init__(self, input_dim, adj_matrix, gcn_hiddens):
        ...
        # last layer (FC layer)
        self.W4 = tf.Variable(init([gcn_hiddens[2], num_classes]))
        self.L4 = tf.matmul(self.L3, self.W4) # shape: (N, num_classes)
        
        # loss
        self.loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.L4, labels=self.y)
        self.loss = self.loss * self.mask # apply mask
        self.loss = tf.reduce_mean(self.loss)
        
        # prediction
        self.pred = tf.argmax(self.L4, axis=1)
    ...
    def fit(self, data, save_dir, learning_rate=0.001, epochs=500, patience=10):
        ...
        y_pred = tf.argmax(self.L4, axis=1)
...
def get_modelname(window_size, after, gcn_hiddens):
    ...
    return 'gcn301_3layer_window{}_after{}_dims{}'.format(window_size, after, gcn_hiddens)
